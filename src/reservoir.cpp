...
void Reservoir::override_water_level(double water_level)
{
    _water_level = water_level < 0 ? 0 : water_level;
}
...
void Reservoir::set_flow_in(double flow_in)
{
    _flow_in = flow_in < 0 ? 0 : flow_in;
}

void Reservoir::set_flow_out(double flow_out)
{
    _flow_out = flow_out < 0 ? 0 : flow_out;
}
